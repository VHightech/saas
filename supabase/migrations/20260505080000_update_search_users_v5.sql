-- Migration: 20260505_update_search_users_v5.sql
-- Description: Updates search_users function to filter by STADIO column instead of stato_contratto.
...
begin
  ...
  with filtered_profiles as (
    ...
    where 
      ...
      AND
      -- Status filter (Using STADIO as per latest request)
      (
        _status_filter = 'all' 
        OR p.stadio = _status_filter
      )
  )
  select 
    fp.id,
    ...
    fp.stadio,
    fp.stato_contratto,
    (select count(*)::int from public.bills b where b.user_id = fp.id)::int as bills_count,
    (select count(*)::int from public.user_supplies us where us.user_id = fp.id)::int as user_supplies_count,
